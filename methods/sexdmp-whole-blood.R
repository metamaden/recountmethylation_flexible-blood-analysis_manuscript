...
md <- md[md$blood.subgroup == "whole_blood",]
dim(md) # 6866   66
...
# 0.098  20.355  35.709  39.104  57.923 122.084
summary(as.numeric(md[md$predsex == "M",]$predage))
# Min. 1st Qu.  Median    Mean 3rd Qu.    Max.
# 0.241  19.235  31.812  37.839  57.292 122.084
summary(as.numeric(md[md$predsex == "F",]$predage))
# Min. 1st Qu.  Median    Mean 3rd Qu.    Max.
# 0.098  21.419  37.879  40.043  58.342 100.465
...
#-----------------------
mval.fname <- "mval-sexdmp_whole-blood_2platforms.csv"
labels.fname <- "samplabels_whole-blood_2platforms.csv"
sep.sym <- ","
mval <- t(getM(gr)) # get mvals
samp.labels.fpath <- file.path(save.dpath, labels.fname) # write labels
fwrite(matrix(rownames(mval), ncol = 1), col.names = F, sep = ",", file = samp.labels.fpath)
# write dnam mvals
mval.fpath <- file.path(save.dpath, mval.fname)
cnv <- matrix(colnames(mval), nrow = 1)
data.table::fwrite(cnv, file = mval.fpath, append = F, col.names = F, row.names = F)
num.samp <- 2000; indexv <- seq(1, nrow(mval), num.samp); t1 <- Sys.time()
for(ri in indexv){
  ...
  data.table::fwrite(as.matrix(mval[start.index:end.index,]), file = mval.fpath, append = T, 
...
sva.results.fname <- "sva-results-sexdmp_wholeblood-fh10k_2-platform.rda"
# load the fh1k matrix
fh10k.fname <- "mval-cgfilt-fh10k_whole-blood_2platforms.csv"
fh10k.fpath <- file.path(save.dpath, fh10k.fname)
